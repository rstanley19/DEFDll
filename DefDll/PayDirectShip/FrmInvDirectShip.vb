...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FrmInvDirectShip))
        ...
        Me.GroupBoxProducts.Size = New System.Drawing.Size(540, 121)
        ...
        Me.UltraGridItems.Size = New System.Drawing.Size(528, 102)
...
#Region "Build this invoice"
    ...
    Private Sub BuildThisInvoice(ByVal InInvoice As Integer, ByVal inVendor As Integer, ByVal InReference As String, ByVal inInvoiceDate As Date)
        ...
        Dim myInvoice As New SharedDll.Iseries.SoInvp(InInvoice)

        If myInvoice.found Then
            '' see if paid in soinvl file

            BillToCustomer = myInvoice.soBlto
            If myInvoice.soStat = "T" Then
                FreightOnlyInvoice = True
            Else
                FreightOnlyInvoice = False
            End If

            Dim myMscustp As New SharedDll.Iseries.msCustp(myInvoice.soAcc_, False)
            ...
            'Load soorddp items
            Dim Daily_QryData As ADODB.Recordset
            Daily_QryData = connAS400.Execute("select oditem, odqshp, odqpd, odpdco, odavgc, imdes1, odlin#, odfroz, ifnull(rate,0) as SuperFund   
             from daily.soorddp  
        ...
        Try
            ...
            For Each iRow As Infragistics.Win.UltraWinGrid.UltraGridRow In UltraGridItems.Rows
                ...
                With mySoApDIP
                    .diVen_ = ApVendor
                    .diRef_ = inReference
                    .diInv_ = Convert.ToInt32(LabInvoice.Text)
                ...
                With mySoOrddp
                    .UpdateQuantityPaid(Convert.ToInt32(LabInvoice.Text), iRow.Cells("Line").Value, iRow.Cells("Complete").Value, iRow.Cells("Cost").Value)
                End With
    ...
    Private Sub UltraGridDocs_MouseUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles UltraGridDocs.MouseUp
        ...
        If Not mouseupCell.Row.Cells("Key").Value = 0 Then
            ...
            If comboBoxDocType.Text = "Invoice" Then
                GetImagePDF(Format(mouseupCell.Row.Cells("Key").Value, "000000000"), True, mouseupCell.Row.Cells("Dir").Value)
...
#Region "Get Tif"
    Private Sub GetImage(ByRef InFile As Long, ByRef InRefreshData As Boolean, ByVal AlreadyHaveDocument As Boolean, ByVal inDir As String)
        ...
        Else
            Dim ws As New com.portspetroleum.www.FuelTrakImage
            'Dim ws As New localhost.FuelTrakImage
            Dim array() As String = Split(ws.GetCodecs(srcFileName), "~")
            LabPages.Text = array(0)
            CurrentPageImageFormat = array(1)
            CurrentPagePixelFormat = array(2)
        End If
    ...
    Private Sub LoadPage(ByVal inPage As Int32)
        ...
        Else
            Dim ws As New com.portspetroleum.www.FuelTrakImage
            'Dim ws As New localhost.FuelTrakImage
            Dim inBytes() As Byte = ws.GetImage(srcFileName)
            Dim Ms As New System.IO.MemoryStream(inBytes)
            RasImage = codecs.Load(Ms, 0, 0, inPage, inPage)
    ...
    Private Sub PictureBox2_MouseDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles PictureBox2.MouseDown
        ...
        Else
            If PictureBox2.ScaleFactor > 0.1 Then PictureBox2.ScaleFactor -= 0.1
        End If
    ...
    Private Sub PictureBoxImageLeft_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PictureBoxImageLeft.Click
        ...
        comboBoxDocType.Text = mySetHandshakeShared.GetDocType(UltraGridDocs.Rows(CurrentDocRow).Cells("Type#").Value)
...
#End Region
    ...
    Private Sub GetImagePDF(ByRef InFile As String, ByRef InType As String, ByVal inDir As String)

        Me.Cursor = System.Windows.Forms.Cursors.WaitCursor
        Try

            srcFileName = inDir & InFile & ".pdf"
            AxAcroPDF1.LoadFile(srcFileName)
            AxAcroPDF1.Visible = True
    ...
    Private Sub ComboBoxDocType_SelectionChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles comboBoxDocType.SelectionChanged
        ...
        With CallGeneric
            ...
            .CommandText = "update daily.soblfxp Set " & _
            "( bxtype)" & _
            " = " & _
            "(" & comboBoxDocType.Value & ") " & _
...
#End Region
    ...
    Private Sub TabControl1_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TabControl1.SelectedIndexChanged
        If TabControl1.SelectedTab.Text = "Notes" Then
            ...
            If lastInvoice = LabInvoice.Text Or _
            ...
            Daily_SohstlQryData = connAS400.Execute("select * from daily.sohstl where ihinv# = " & LabInvoice.Text, Parms, -1)
            LoadListBoxNotes((ListBoxNotes), (Daily_SohstlQryData))
...
#Region "Miscellaneous Charge"
    ...
    Private Function EditAdditionalCharge() As Boolean
        ...
        Dim pRow As Infragistics.Win.UltraWinGrid.UltraGridRow = UltraGridMisc.Rows(0)
        Do Until pRow Is Nothing
            If Len(Trim(pRow.Cells("Amount").Value)) > 0 Or _
